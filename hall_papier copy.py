...
def secant_method(F_Te, z, G_bar, Gamma_bar, Te_0, Te_1, tolerance=1e-6, max_iter=100):
...
def RHS_Anna_HET(z_bar, y): #z corresponds to the distance along the channel, y is the vector of the unknowns
    ...
    Te_1_bar = secant_method(F_Te, z, G_bar, Gamma_bar, Te_0, Te_1)
    
    print("z: "+ str(z_bar))
        
    K_iz = K_iz_star * ((3/2 * Te_1_bar)**0.25) * np.exp(-4/(3 * Te_1_bar))    
    f_iz = K_iz/K_iz_star
    f_ce = np.exp(-C_mag * (z_bar - 1)**2) #(45)
...
z_eval = np.linspace(0, 0.7, 1000)
sol = solve_ivp(RHS_Anna_HET, [0, 0.7], [Gamma_0_bar, G_0_bar], method='RK45', rtol=1e-5, atol=[1e-5, 1e-5], t_eval = z_eval) #solution of the system of ODEs with initial conditions Gamma_0 and G_0
...
# recalculate Te
Te_bar_values = np.zeros(N0)
for i in range(N0):
    Te_0 = 0.5 * E_iz
    Te_1 = Te_0 + 0.1 * E_iz   
    Te_0_bar = secant_method(F_Te, z_bar[i] * L_ch, G_bar[i], Gamma_bar[i], Te_0, Te_1)
    Te_bar_values[i] = Te_0_bar
...
#     F_Te_0 = F_Te(Te_0, z[i], G_bar[i], Gamma_bar[i])
#     F_Te_1 = F_Te(Te_1, z[i], G_bar[i], Gamma_bar[i])
        
#     while np.abs(F_Te_0) > 1e-5:
#         Te_2 = Te_1 - F_Te_1 * (Te_1 - Te_0) / (F_Te_1 - F_Te_0)        
#         Te_0, Te_1 = Te_1, Te_2
#         F_Te_0, F_Te_1 = F_Te_1, F_Te(Te_2, z_bar[i], G_bar[i], Gamma_bar[i])
    
#     Te_bar_values[i] = Te_0 / E_iz

#recalculate the Te
Te_bar_values = np.array(Te_bar_values)
...
K_iz = K_iz_star * ((3 * Te_bar_values / (2 * E_iz))**0.25) * np.exp(-4 * E_iz / (3 * Te_bar_values))
...
plt.figure(2)
plt.plot(z_bar, Te_bar_values, 'b', linewidth=1)
plt.xlim([0, 1])
plt.ylim([0, 25])
plt.xlabel('$x/L_{\\rm ch}$', fontsize=14)
